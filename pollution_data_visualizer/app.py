...
def collect_all_data(force=False):
...
def get_current():
    ...
    try:
        collect_data(city, max_age_minutes=0)
        latest = PollutionRecord.query.filter_by(city=city).order_by(PollutionRecord.timestamp.desc()).first()
        if not latest:
            return jsonify({'error': 'no data'}), 404
        return jsonify({
            'city': city,
